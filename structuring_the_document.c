...
{
    struct paragraph * paragraph_obj = NULL;
    struct sentence * sentence_obj = NULL;
    char * word = NULL;
    ...
    while(*s != '\0')
    {
        if(paragraph_obj == NULL)
        {
            ...
            paragraph_obj = doc.data + doc.paragraph_count - 1;
            
            paragraph_obj->data = NULL;
            paragraph_obj->sentence_count = 0;
            
            sentence_obj = NULL;
        }
        if(sentence_obj == NULL)
        {
            paragraph_obj->sentence_count++;
            paragraph_obj->data = (struct sentence*)realloc(paragraph_obj->data,sizeof(struct sentence) * paragraph_obj->sentence_count);
            
            
            sentence_obj = paragraph_obj->data + paragraph_obj->sentence_count - 1;
            
            sentence_obj->data = NULL;
            sentence_obj->word_count = 0;
        ...
        else if(word == NULL)
        {
            ...
            sentence_obj->word_count++;
            sentence_obj->data = (struct word*)realloc(sentence_obj->data,sizeof(struct word) * sentence_obj->word_count);
            word = s;
            sentence_obj->data[sentence_obj->word_count - 1].data = word;
        }
...
int main() 
{
	printf("\nHow many data you want to enter AND then enter the data\n\n ");
    char* text = get_input_text();
    struct document Doc = get_document(text);

    int q;
	printf("\nHow many variation you want to search\n");
    scanf("%d", &q);
    ...
    while (q--) {
        ...
        if (type == 3){
            int k, m, n;
			printf("\nEnter the k'th(word) in m'th(sentence)of n'th(paragraph) \n");
            scanf("%d %d %d", &k, &m, &n);
            ...
            int k, m;
			printf("\nEnter the k'th(sentence) in m'th(paragraph) \n");
            scanf("%d %d", &k, &m);
