...
while(*s != '\0')
{
    if(para == NULL)
    {
        ...
        doc = (char****)realloc(doc,sizeof(char***) * paragraph_count);
        
        *(doc + paragraph_count - 1) = (char***)malloc(sizeof(char**));
        
        paragraph = *(doc + paragraph_count - 1);
        para = *(doc + paragraph_count - 1);
    ...
    if(sent == NULL)
    {
        ...
        paragraph = (char***)realloc(paragraph,sizeof(char**) * sentence_count);
        
        *(doc + paragraph_count - 1) = paragraph;
        
        *(paragraph + sentence_count - 1) = (char**)malloc(sizeof(char*));
        sentence = *(paragraph + sentence_count - 1);
       
        sent = *(paragraph + sentence_count - 1);
    ...
    if(*s == ' ' || *s == '.' )
    {
        ...
        sentence = (char**)realloc(sentence,sizeof(char*) * word_count);
        
        *(paragraph + sentence_count - 1) = sentence;
        
        // printf("3:  %ld        %ld        %ld\n",doc,paragraph,sentence);
        
        
        sentence[word_count - 1] = start; 
